...
### Focused setup and status modes

Use an explicit mode when the full bootstrap is not wanted. These commands do
not run unrelated bootstrap work:

```bash
python3 install.py --models  # optional model environments and weights only
python3 install.py --daemon  # retention-daemon artifact only; never enables it
python3 install.py --all     # both of the above, and nothing else
python3 install.py --check   # report model and daemon status; change nothing
```

`--apply` remains the backward-compatible full setup command. `--verify`
remains its legacy verification command. `--no-models` and `--no-forecaster`
are legacy `--apply` modifiers, not focused-mode options.
...
|---|---|---:|---:|---:|---:|---|
| [`macbook-air-m1/`](macbook-air-m1/human-readable/REPORT.md) | Apple M1 · 8 cores · 16 GB | 8 | **54.71B** | 18.49B | 24.36B | 2026-08-27 |
| `dell-inspiron-desktop-linux/` | — | — | — | — | — | 📒 **ledger only** — 11.29M, 7 sessions |
| `hp-laptop-linux/` | — | — | — | — | — | 📒 **ledger only** — 13.37B, 374 sessions |
| `dell-latitude-7480-linux/` | — | — | — | — | — | 📒 **ledger only** — 4.46B, 13,378 sessions |
| `dell-latitude-7480-windows/` | — | — | — | — | — | ❌ never |
| `asus-laptop-linux/` | — | — | — | — | — | 📒 **ledger only** — 146.98M, 36 sessions |
| **All computers** | | | **54.71B** | **18.49B** | | |
...
The append-only token ledgers stand behind **42.36B** across the fleet, of which **9.65M** is usage no scan can still see, because the transcripts behind it have been deleted. `LIFETIME.md` counts it; the columns above do not. **17.99B** of that is on machines with a ledger and no scan at all — `asus-laptop-linux`, `dell-inspiron-desktop-linux`, `dell-latitude-7480-linux`, `hp-laptop-linux` — which is not the state above and not never scanned: the tokens are recorded, they are in no column here, and how much of them a scan could still see is unknown rather than zero.

_1 of 6 computers scanned; generated by `combine.py` 2026-08-27. Do not edit by hand._
<!-- END OVERVIEW -->
...
|---|---:|---:|---|
| broodierchip@gmail.com | 12.83B | 69.4% | 1 |
| codehunterextreme@gmail.com | 5.32B | 28.8% | 1 |
| alexander.sorrell.it@gmail.com | 322.28M | 1.7% | 1 |
| unknown (Documents) | 10.01M | 0.1% | 1 |

_4 account(s) across 1 scanned computer(s), 18.49B total. Generated by `combine.py`. Do not edit by hand._
<!-- END ACCOUNTS -->
...
| by CLI | | | by company | |
|---|---:|---|---|---:|
| claude | 19.66B | | anthropic | 20.10B |
| codex | 1.63B | | openai | 1.71B |
| gemini | 995.36M | | google | 1.57B |
| bob | 802.95M | | - | 802.95M |
| antigravity | 597.49M | | xai | 100.22M |
| copilot | 572.45M | | synthetic | 59.37M |
| grok | 100.22M | | antigravity | 18.12M |
| lmstudio | 53.16K | | other | 53.16K |
| kilocode | 0 | | | |
| clawspring | 0 | | | |
| copilot-chat | 0 | | | |
| claude-orphans | 0 | | | |

_Generated by `combine.py` from 1 scanned machine(s); 24.36B across 12 CLI(s). Do not edit by hand._
<!-- END CLI -->
...
# 2. scan this computer and write BOTH folders' NUMBERS
#    (this does NOT export transcripts — step 4 does, and it is separate)
python3 run.py update

# 3. push this computer's numbers
git commit -m "scan <machine>" && git pull --rebase && git push

# 4. EXPORT the transcripts, then push them as a release asset, NOT through git
#    export_corpus.py is not called by `run.py update`. Skipping it packs
#    whatever corpus/ happens to hold — on a machine that has never exported,
