...
Submitted from a terminal with the network on, `npx standout standout-founding-eng`:

```
  YOU MANAGED        896 session-hours (top 3% of users)
  YOUR AI CODING YEAR ~1,533 hours · 16,800 sessions since 2026-01
                      Jul 26 ████████████████████ 966h   ← best month
  TOKENS             12.7B (top 5%) · 111.2M generated fresh
  LOYALIST           CLAUDE CODE · 11,622 sessions · 44-day streak
  TOP PROJECTS       ▸ Workspace  11,621 sessions
  WHEN YOU CODE      night owl, peak 22:00 · weekdays 69% / weekends 31%
  AGENTS             ~4.7 at once (top 9%) · peak 10 · longest tab 15 days
  MODELS             Opus 4.8 — 99% of sessions
  HOW YOU TALK       no message sample provided.
                     19,790 prompts · 23% questions · ~3786 chars
                     recurring: no sample available
  ──  UNICORN  ──    100 / 100 · top 1% of 3,756 users
                     intensity 89 · consistency 99 · craft +7
...
`standout.work/w/dev-4xy9`

**Two of those cards are wrong for reasons worth naming.**

`TOP PROJECTS ▸ Workspace 11,621 sessions` — one project out of 401. That is
**our** doing: the corpus rewrites every `cwd` to `/workspace` to protect
private repository names, and their derivation collapses them. Not a defect in
their tool.

`HOW YOU TALK → no message sample provided` — that one is theirs, and it was
predicted from reading `capPayload` before the run happened. See the payload
section above.
