...
export class MutationTestReportHelper {
    ...
    static inject = tokens(coreTokens.reporter, commonTokens.options, coreTokens.project, commonTokens.logger, coreTokens.testCoverage, coreTokens.fs, coreTokens.requireFromCwd, coreTokens.unexpectedExitRegistry);
    constructor(reporter, options, project, log, testCoverage, fs, requireFromCwd, unexpectedExitHandler) {
        ...
        unexpectedExitHandler.registerHandler(async () => {
            if (this.options.incremental &&
                ...
                this.log.info('Saved a partial incremental report to "%s" after an unexpected interrupt.', this.options.incrementalFile);
            }
        });
    }
    ...
    async writeIncrementalReport(report) {
    ...
    async mutationTestReport(results) {
        ...
        const testIdMap = new Map([...this.testCoverage.testsById.values()].map((test, index) => [
            test.id,
            index.toString(),
        ]));
    ...
    async toTestFiles(remapTestId) {
        const testFilesByName = new Map(await Promise.all([
            ...new Set([...this.testCoverage.testsById.values()].map(({ fileName }) => fileName)),
        ].map(async (fileName) => [
    ...
    discoverDependencies() {
        const discover = (specifier) => {
            try {
                return [
                    specifier,
                    this.requireFromCwd(`${specifier}/package.json`).version,
                ];
            }
            catch {
...
export function normalizeReportFileName(fileName) {
    if (fileName) {
