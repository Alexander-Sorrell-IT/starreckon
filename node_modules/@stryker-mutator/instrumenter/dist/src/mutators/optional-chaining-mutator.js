 ...
 * foo?.() -> foo()
 */
export const optionalChainingMutator = {
    ...
    *mutate(path) {
        if (path.isOptionalMemberExpression() && path.node.optional) {
            ...
            if (isPartOfOptionalChain(path.node.object)) {
                yield t.optionalMemberExpression(obj, prop, path.node.computed, 
                /*optional*/ false);
            }
            else {
        ...
        if (path.isOptionalCallExpression() && path.node.optional) {
            ...
            if (isPartOfOptionalChain(path.node.callee)) {
                yield t.optionalCallExpression(t.cloneNode(path.node.callee, true), args, 
                /*optional*/ false);
            }
            else {
...
function isPartOfOptionalChain(exp) {
