# Claude Code token usage — Dell Latitude 7480 Linux

_Generated 2026-08-09T02:48:25-05:00_

## Total for this computer: 2,353,868,873 tokens (2.35B)

Across 12 account(s), 33,076 sessions, 29,677 assistant turns.

Counted from `message.usage` in the local session JSONL — the API's own
accounting, deduplicated by message uuid.

## Accounts

| Account | Sessions | Turns | Input | Cache write | Cache read | Output | Total |
|---|---:|---:|---:|---:|---:|---:|---:|
| owner@example.com | 62 | 3,677 | 167.64K | 33.42M | 869.72M | 4.57M | **907.88M** |
| second@example.com | 16,441 | 17,594 | 159.91M | 122.82M | 433.36M | 8.67M | **724.76M** |
| unknown (old_.claude) | 31 | 8,254 | 513.10K | 49.64M | 654.43M | 1.38M | **705.96M** |
| third@example.com | 2 | 99 | 3.56K | 1.44M | 10.27M | 75.34K | **11.79M** |
| user:2d4777822844 | 1 | 50 | 174 | 506.20K | 2.93M | 52.28K | **3.49M** |
| user:283b8e5b8e48 | 1 | 3 | 0 | 0 | 0 | 0 | **0** |
| unknown (claude) | 16,441 | 0 | 0 | 0 | 0 | 0 | **0** |
| unknown (claude-alt) | 2 | 0 | 0 | 0 | 0 | 0 | **0** |
| unknown (claude-alt-api) | 1 | 0 | 0 | 0 | 0 | 0 | **0** |
| unknown (claude-api) | 1 | 0 | 0 | 0 | 0 | 0 | **0** |
| unknown (claude-it) | 62 | 0 | 0 | 0 | 0 | 0 | **0** |
| second@example.com | 31 | 0 | 0 | 0 | 0 | 0 | **0** |

## By provider

Claude Code can be pointed at a non-Anthropic backend; those transcripts are
byte-identical to Claude ones. Totals are split on the model id so an
Anthropic figure is never inflated by tokens Anthropic did not serve.

| Provider | Tokens | Share |
|---|---:|---:|
| anthropic | 2,353,868,873 | 100.0% |

**Anthropic-only total: 2,353,868,873 tokens (2.35B)**


### Other AI tools on this machine

Listed so a provider missing from the table above is unambiguous: a tool
that records no usage cannot be counted from disk, which is different
from a tool that was never used.

| Tool | Directory | Files | Token usage on disk |
|---|---|---:|---|
| Gemini CLI | `/home/phantom-orchestrator/.gemini` | 103 | **no — not countable** |
| GitHub Copilot CLI | `/home/phantom-orchestrator/.copilot` | 205 | **no — not countable** |
| Antigravity CLI | `/home/phantom-orchestrator/.antigravitycli` | 1 | **no — not countable** |
| OpenAI Codex CLI | `/home/phantom-orchestrator/.codex` | 282 | yes — countable |

## Authentication and organization

An API-key profile bills to the organization the key belongs to, which is
not recorded on disk — rerun with `--probe-api` to resolve it. A profile is
linked to an account only when their organization UUIDs match.

| Account | Auth | Organization | Org UUID | Linked to |
|---|---|---|---|---|
| owner@example.com | oauth | owner@example.com's Organization | `66565064-263e-4d54-b06f-15cae4b2febc` | — |
| second@example.com | oauth | second@example.com's Organization | `6bfc754e-a205-41c3-9dac-80e593f494e0` | — |
| unknown (old_.claude) | unknown | — | `—` | — |
| third@example.com | oauth | third@example.com's Organization | `ba511861-53b4-4599-8019-93db6389d991` | — |
| user:2d4777822844 | api_key | — | `—` | — |
| user:283b8e5b8e48 | api_key | — | `—` | — |
| unknown (claude) | unknown | — | `—` | — |
| unknown (claude-alt) | unknown | — | `—` | — |
| unknown (claude-alt-api) | unknown | — | `—` | — |
| unknown (claude-api) | unknown | — | `—` | — |
| unknown (claude-it) | unknown | — | `—` | — |
| second@example.com | oauth | second@example.com's Organization | `6bfc754e-a205-41c3-9dac-80e593f494e0` | — |

### owner@example.com

**907,878,994 tokens** (907.88M) — `/home/phantom-orchestrator/.claude-it`

userID `3572682e824fbb759bad843c8552a4fa8dd47736a85bfc94a44fecdfa8fc346b`

| Transcript | Files | Tokens | Share |
|---|---:|---:|---:|
| main | 62 | 891.97M | 98% |
| subagent | 10 | 15.91M | 2% |

| Model | Total |
|---|---:|
| claude-opus-4-8 | 447.68M |
| claude-opus-5 | 246.32M |
| claude-fable-5 | 203.77M |
| claude-opus-4-7 | 10.11M |

Active 2026-07-04 → 2026-08-09 (28 days). Busiest day 2026-07-23 at 106.65M.

| Project | Total |
|---|---:|
| `-home-phantom-orchestrator` | 639.11M |
| `-home-phantom-orchestrator-tv-tracker` | 117.54M |
| `-media-phantom-orchestrator-BitcoinNode-AI-Projects-MultiBoot` | 86.27M |
| `-home-phantom-orchestrator-questline` | 51.66M |
| `-media-phantom-orchestrator-Elements1-Build-From-Scratch-Ideas-look-at-this-Cognitive-profile` | 11.10M |
| `-home-phantom-orchestrator-token-usage` | 1.97M |
| `-home-phantom-orchestrator-Documents-Transcript-engine` | 231.75K |

### second@example.com

**724,756,400 tokens** (724.76M) — `/home/phantom-orchestrator/.claude`

userID `08537de04fe9cd9f989aa2c5fa9ffa076ee3f6a0286fc671f5c7a6016fc374b3`

| Transcript | Files | Tokens | Share |
|---|---:|---:|---:|
| main | 16,441 | 710.76M | 98% |
| workflow | 15 | 14.00M | 2% |

| Model | Total |
|---|---:|
| claude-opus-4-8 | 612.71M |
| claude-opus-5 | 110.62M |
| claude-haiku-4-5-20251001 | 1.20M |
| claude-opus-4-6 | 223.25K |

Active 2026-07-03 → 2026-08-02 (19 days). Busiest day 2026-07-11 at 84.83M.

| Project | Total |
|---|---:|
| `-media-phantom-orchestrator-Elements1-logistis-app` | 583.43M |
| `-home-phantom-orchestrator-Documents-Music` | 132.07M |
| `-home-phantom-orchestrator` | 7.72M |
| `-tmp` | 1.20M |
| `-media-phantom-orchestrator-Elements1-logistis-App` | 344.65K |

### unknown (old_.claude)

**705,961,350 tokens** (705.96M) — `/home/phantom-orchestrator/.ai-logs-archive/claude/old_.claude`

| Transcript | Files | Tokens | Share |
|---|---:|---:|---:|
| main | 31 | 580.79M | 82% |
| subagent | 275 | 125.17M | 18% |

| Model | Total |
|---|---:|
| claude-opus-4-6 | 433.44M |
| claude-opus-4-5-20251101 | 225.02M |
| claude-haiku-4-5-20251001 | 38.46M |
| claude-sonnet-4-5-20250929 | 6.44M |
| claude-sonnet-4-6 | 2.59M |

Active 2026-01-17 → 2026-02-24 (21 days). Busiest day 2026-02-22 at 233.48M.

| Project | Total |
|---|---:|
| `-media-phantom-orchestrator-BitcoinNode-AI-Projects-MultiBoot` | 288.67M |
| `-media-phantom-orchestrator-BitcoinNode-AI-Projects-Bug-Solving` | 83.06M |
| `-home-phantom-orchestrator` | 82.75M |
| `-media-phantom-orchestrator-BitcoinNode-gemini-crazy-idea-forta` | 73.60M |
| `-media-phantom-orchestrator-BitcoinNode-gemini-crazy-idea-real-math-solving-KS-COMPRESS-ENCRYPT-SYSTEM` | 56.82M |
| `-media-phantom-orchestrator-BitcoinNode-Claude-workspace` | 48.64M |
| `-media-phantom-orchestrator-BitcoinNode-Claude-workspace-BIOS-upgrade` | 26.63M |
| `-media-phantom-orchestrator-BitcoinNode-copilot` | 18.56M |

### third@example.com

**11,786,126 tokens** (11.79M) — `/home/phantom-orchestrator/.claude-alt`

userID `2b2f79380228ba53af8e39adbd917e1bdd126de65caf3d62a42d62f349fd9473`

| Transcript | Files | Tokens | Share |
|---|---:|---:|---:|
| main | 2 | 11.79M | 100% |

| Model | Total |
|---|---:|
| claude-opus-4-8 | 11.79M |

Active 2026-07-25 → 2026-07-26 (2 days). Busiest day 2026-07-25 at 8.90M.

| Project | Total |
|---|---:|
| `-home-phantom-orchestrator-Documents-Transcript-engine` | 11.79M |

### user:2d4777822844

**3,486,003 tokens** (3.49M) — `/home/phantom-orchestrator/.claude-alt-api`

userID `2d4777822844690431e6e0c2b862bc04d5388b9850bcdef441c6342683a1c1d2`

| Transcript | Files | Tokens | Share |
|---|---:|---:|---:|
| main | 1 | 3.49M | 100% |

| Model | Total |
|---|---:|
| claude-sonnet-4-6 | 3.49M |

Active 2026-04-24 → 2026-04-24 (1 days). Busiest day 2026-04-24 at 3.49M.

| Project | Total |
|---|---:|
| `-media-phantom-orchestrator-Elements1-Final-API-alt` | 3.49M |

### user:283b8e5b8e48

**0 tokens** (0) — `/home/phantom-orchestrator/.claude-api`

userID `283b8e5b8e48d17836c99894017f4ce322b58772226367f320d5373a1104bd43`

### unknown (claude)

**0 tokens** (0) — `/home/phantom-orchestrator/.ai-logs-archive/claude/claude`

### unknown (claude-alt)

**0 tokens** (0) — `/home/phantom-orchestrator/.ai-logs-archive/claude/claude-alt`

### unknown (claude-alt-api)

**0 tokens** (0) — `/home/phantom-orchestrator/.ai-logs-archive/claude/claude-alt-api`

### unknown (claude-api)

**0 tokens** (0) — `/home/phantom-orchestrator/.ai-logs-archive/claude/claude-api`

### unknown (claude-it)

**0 tokens** (0) — `/home/phantom-orchestrator/.ai-logs-archive/claude/claude-it`

### second@example.com

**0 tokens** (0) — `/home/phantom-orchestrator/old/.claude`

userID `08537de04fe9cd9f989aa2c5fa9ffa076ee3f6a0286fc671f5c7a6016fc374b3`
